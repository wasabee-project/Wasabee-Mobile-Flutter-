...
class AlertsPage {
  static Widget getPageContent(
      List<TargetListViewModel> listOfVM, List<Target> listOfTargets, MapPageState mapPageState) {
    return Scaffold(
              ...
              children: <Widget>[
                Expanded(
                    child: DropdownButton<AlertFilterType>(
                      isExpanded: true,
                  value: mapPageState.alertsSortDropdownValue,
                  underline: Divider(
                    color: Colors.white,
                  ),
                  onChanged: (AlertFilterType newValue) {
                    LocalStorageUtils.setAlertfilter(newValue).then((any) {
                      mapPageState.setAlertSortDropdownValue(newValue);
                    });
                  },
                  items: AlertFilterManager.getFilters()
                    ...
                    return DropdownMenuItem<AlertFilterType>(
                      value: value,
                      child: Text(AlertFilterManager.getDisplayStringFromEnum(value, listOfTargets, mapPageState.googleId)),
                    );
                  }).toList(),
                )),
                Row(
                ...
                IconButton(color: Colors.white, icon: Icon(Icons.sort, color: Colors.black,), onPressed: () {},),
              ])),
      Divider(
        color: Colors.black,
        height: 4,
